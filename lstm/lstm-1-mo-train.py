...
for idx in range(len(validation_start_index)):
    ...
    for h in hyperparam:
        ...
        regressor.add(LSTM(units = 50, return_sequences = True, input_shape = (30,69)))
        ...
        for i in range(len(train_data)):
            ...
            # Obtain indices of blocks of non-overlapping rows of 30
            block_indices = list(range(30, len(company_train_data_X), 30))

            X_train = []
            y_train = []

            # For each closing 6-month price (y), create a 3D vector of 30 rows, and 84 columns
            for j in block_indices:
                X_train.append(company_train_data_X[j - 30: j])
        ...
        for i in range(start_index, end_index):
            ...
            # Obtain indices of blocks of non-overlapping rows of 30
            block_indices = list(range(30, len(company_validation_data_X), 30))
            ...
            # For each closing 6-month price (y), create a 3D vector of 30 rows, and 84 columns
            for j in block_indices:
                X_validation.append(company_validation_data_X[j - 30: j])
...
# Write the average error for each hyperparamter (N=1352)

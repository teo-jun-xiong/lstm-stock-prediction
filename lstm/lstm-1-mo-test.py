...
for i in range(len(train_data)):
    ...
    block_indices = list(range(30, len(company_test_data_X), 30))

    X_train = []
    y_train = []

    # For each closing 1-month price (y), create a 3D vector of 30 rows, and 84 columns
    for j in block_indices:
        X_train.append(company_test_data_X[j - 30: j])
...
companies = os.listdir('./data/1mo_train')
...
for i in range(len(test_data)):
    ...
    # For each closing 1-month price (y), create a 3D vector of 30 rows, and 84 columns
    for j in block_indices:
        X_test.append(company_test_data_X[j - 30: j])
    ...
    plt.savefig('./plots/1-month/'+name)
    plt.close()
